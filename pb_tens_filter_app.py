...
def main():
    ...
    prev2    = st.sidebar.text_input("Prev tens (Draw 2-back, optional):").strip()
    prev3    = st.sidebar.text_input("Prev-prev tens (Draw 3-back, optional):").strip()
    ...
    prev_digits = digits_from_str(prev2)
    prev3_digits = digits_from_str(prev3)
    prev4_digits = digits_from_str(prev4)
    prev5_digits = digits_from_str(prev5)
    prev6_digits = digits_from_str(prev6)
    ...
    history_digits = seed_digits+prev_digits+prev3_digits+prev4_digits+prev5_digits+prev6_digits
    ...
    hot_digits  = manual_hot or hot_auto
    cold_digits = manual_cold or cold_auto
    due_digits  = manual_due or (None if disable_due else due_auto)
    ...
    if tracked:
        ...
        for t in tracked:
            ...
            elif any(t in eliminated for eliminated in dyn_counts): st.warning(f"{t} eliminated")
            else: st.info(f"{t} not generated")
